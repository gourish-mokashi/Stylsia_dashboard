...
export function useAdminBrands(): UseAdminBrandsReturn {
    ...
    try {
      ...
      const brandsWithStats: BrandWithStats[] = await Promise.all(
        (brandsData || []).map(async (brand) => {
          ...
          if (countError) {
            console.warn(
              `Failed to get product count for brand ${brand.id}:`,
              countError
            );
          }
          ...
          if (productError && productError.code !== "PGRST116") {
            console.warn(
              `Failed to get last activity for brand ${brand.id}:`,
              productError
            );
          }
  ...
  const updateBrandStatus = useCallback(
    async (brandId: string, status: DatabaseBrand["status"]) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to update brand status"
        );
        throw err;
      }
    },
